@extends('layouts.base')

@section('title', 'Ресурсы')

@section('content')


    <div class="cards md:w-3/4 p-4">

        <h2>Ресурсы</h2>

        <ul class="flex flex-wrap justify-between">
            @foreach($posts as $post)
                <li class="w-4/5  md:w-[30%] mr-2 mb-6 rounded-lg shadow-2xl hover:scale-110 transition-all duration-800">

                    <div>
                        <a href="{{ route('resources.single', ['slug' => $post->slug]) }}">
                            <img class="rounded-lg" src="{{ $post->getImage() }}" alt="main post image">
                        </a>
                        <div class="text-gray-500 p-2">
                            <a href="{{ route('posts.single', ['slug' => $post->slug]) }}">
                                <h3 class="text-xl font-semibold uppercase m-2">{{ $post->title }}</h3>
                            </a>
                            <p class="text-sm text-black-50">{!! $post->description !!}</p>
                            <a href="{{ route('categories.single', ['slug' => $post->category->slug]) }}" class="">{{ $post->category->title }}</a>
                        </div>
                    </div>

                </li>
            @endforeach
        </ul>

        <div class="pagination">
            {{ $posts->onEachSide(1)->links() }}
        </div>

    </div>

@endsection
